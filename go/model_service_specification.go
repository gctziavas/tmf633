...
// ServiceSpecification - ServiceSpecification is a class that offers characteristics to describe a type of service. Functionally, it acts as a template by which Services may be instantiated. By sharing the same specification, these services would therefore share the same set of characteristics.
...
// AssertServiceSpecificationRequired checks if the required fields are not zero-ed
func AssertServiceSpecificationRequired(obj ServiceSpecification) error {
...
// AssertRecurseServiceSpecificationRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of ServiceSpecification (e.g. [][]ServiceSpecification), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseServiceSpecificationRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aServiceSpecification, ok := obj.(ServiceSpecification)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertServiceSpecificationRequired(aServiceSpecification)
	})
}
